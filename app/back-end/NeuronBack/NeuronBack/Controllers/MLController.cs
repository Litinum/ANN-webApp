...
namespace NeuronBack.Controllers
{
    ...
    public class MLController : ControllerBase
    {
        ...
        public async Task<IActionResult> SaveModel([FromBody]dynamic mod)
        {
            try
            {
                int id = mod.experimentid;
                Console.WriteLine();
                var experiment = _context.Experiments.Where(x => x.id == id).FirstOrDefault();

                if(experiment != null)
                {
                    dynamic t = new System.Dynamic.ExpandoObject();
                    string ff = JsonConvert.SerializeObject(mod.Filename); 
                    t.Filename = experiment.experimentName + "/" + ff.Replace("\"", "");
                    string temp = JsonConvert.SerializeObject(t);
                    Console.WriteLine("ZZZZZZ " + temp);
                    string path = pythonConnection.SaveModel(temp);
                    // SERVER
                    // string filename = path.Split('/').Last();
                    Console.WriteLine("SV " + path);
                    string filename = path.Split("\\").Last();
                    filename = Path.GetFileName(path);
                    //string path = "putanja";
        ...
        public IActionResult LoadModel([FromBody] dynamic mode)
        {
            try
            {
                ...
                var experiment = _context.Experiments.Where(x => x.id == model.experimentid).FirstOrDefault();
                string tempPath = Path.Join(experiment.experimentName, model.modelName);
                Console.WriteLine(experiment.experimentName);
                Console.WriteLine(model.modelName);
                string ss = experiment.experimentName + "\\" + model.modelName;
                Console.WriteLine(ss);
                string baseUrl = GLOBAL_DOWNLOAD_URL;
                string json = "{ \"Model\" : \"" + baseUrl + model.path + "&path=" + tempPath + "\"}";
                Console.WriteLine(json);
        ...
        [HttpPost]
        [Route("downloadFileP")]
        public FileResult DownloadFile(string filename, string? path = null)
        {
            ...
            byte[] bytes;
            if (path == null)
                bytes = FileManager.DownloadFileFromServer(userName, FileManager.DefaultExperimentFolder, filename);
            else
                bytes = FileManager.DownloadFileFromServer(userName, path, filename);
        ...
        public IActionResult UploadFilePython(string? path = null)
        {
            ...
            try
            {
                ...
                if (path != null)
                    newPath = FileManager.UploadFileToServer(userName, path, file);
                else
